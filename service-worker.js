self.addEventListener("install", (event) => {
  event.waitUntil(
    caches.open("walk-with-god-v47").then((cache) =>
      cache.addAll([
        "/",
        "/index.html",
        "/why-we-walk.html",
        ...
        "/styles.css?v=20260714-step-labels",
        "/app.js?v=20260714-step-labels",
...
self.addEventListener("activate", (event) => {
  event.waitUntil(
    caches.keys()
      .then((keys) => Promise.all(keys.filter((key) => key !== "walk-with-god-v47").map((key) => caches.delete(key))))
