self.addEventListener("install", (event) => {
  event.waitUntil(
    caches.open("walk-with-god-v68").then((cache) =>
      cache.addAll([
        "/",
        "/index.html",
        "/why-we-walk.html",
        ...
        "/styles.css?v=20260802-breathwork-9",
        "/app.js?v=20260802-breathwork-9",
...
self.addEventListener("activate", (event) => {
  event.waitUntil(
    caches.keys()
      .then((keys) => Promise.all(keys.filter((key) => key !== "walk-with-god-v68").map((key) => caches.delete(key))))
  ...
  event.respondWith(
    fetch(event.request)
      .then((response) => {
        ...
        caches.open("walk-with-god-v68").then((cache) => cache.put(event.request, copy)).catch(() => undefined);
        return response;
      })
