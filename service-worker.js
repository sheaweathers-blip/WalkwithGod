self.addEventListener("install", (event) => {
  event.waitUntil(
    caches.open("walk-with-god-v40").then((cache) =>
      cache.addAll([
        "/",
        "/index.html",
        "/privacy.html",
        ...
        "/styles.css?v=20260709-next-step-language",
        "/app.js?v=20260709-next-step-language",
...
self.addEventListener("activate", (event) => {
  event.waitUntil(
    caches.keys()
      .then((keys) => Promise.all(keys.filter((key) => key !== "walk-with-god-v40").map((key) => caches.delete(key))))
  ...
  event.respondWith(
    fetch(event.request)
      .then((response) => {
        ...
        caches.open("walk-with-god-v40").then((cache) => cache.put(event.request, copy)).catch(() => undefined);
        return response;
      })
